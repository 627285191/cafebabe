...
//							var value = eval(objName);
							var value = getValue.call(obj, objName);
						...
						if (value != null) {  //这里稍加修改，如果不设置type属性，就获取不到该元素的类型了。追加$(item)[0].type
								switch ($(item).attr("type")||$(item)[0].type) {
								case "hidden":
								case "password":
								case "textarea":
									$(item).val(value);
	...
	$.fn.reset = function(){
		this.get(0).reset ? this.get(0).reset() : null;
		return this;
	};
